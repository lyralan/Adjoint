...
!
     ...
     &  'G F E D 3   B I O M A S S   B U R N I N G   E M I S S I O N S'
      ...
      IF ( MM < 10 ) THEN
         WRITE( MONTH_STR, '(i1)' )  MM
      ELSE
         WRITE( MONTH_STR, '(i2)' )  MM
      ENDIF

      ! File name with GFED3 DM emissions
      FILENAME1 = '/net/d13/data/kingshuk/GFED4/2015/' //
     &            'GFED4_' // TRIM(MONTH_STR) // '_05x05.nc'
      ...
      CALL Ncop_Rd( fId1,TRIM(FILENAME1) )
      CALL NcRd( ARRAY_GFED3,fId1,'DMAGRI',st2d,ct2d )
      CALL NcCl( fId1 )

      DM_GFED3_MON(:,:,1) = ARRAY_GFED3(:,:,1)

      CALL Ncop_Rd( fId1,TRIM(FILENAME1) )
      CALL NcRd( ARRAY_GFED3,fId1,'DMBORF',st2d,ct2d )
      CALL NcCl( fId1 )

      DM_GFED3_MON(:,:,2) = ARRAY_GFED3(:,:,1)

      CALL Ncop_Rd( fId1,TRIM(FILENAME1) )
      CALL NcRd( ARRAY_GFED3,fId1,'DMDEFO',st2d,ct2d )
      CALL NcCl( fId1 )

      DM_GFED3_MON(:,:,3) = ARRAY_GFED3(:,:,1)

      CALL Ncop_Rd( fId1,TRIM(FILENAME1) )
      CALL NcRd( ARRAY_GFED3,fId1,'DMPEAT',st2d,ct2d )
      CALL NcCl( fId1 ) 

      DM_GFED3_MON(:,:,4) = ARRAY_GFED3(:,:,1)

      CALL Ncop_Rd( fId1,TRIM(FILENAME1) )
      CALL NcRd( ARRAY_GFED3,fId1,'DMSAVA',st2d,ct2d )
      CALL NcCl( fId1 ) 

      DM_GFED3_MON(:,:,5) = ARRAY_GFED3(:,:,1)

      CALL Ncop_Rd( fId1,TRIM(FILENAME1) )
      CALL NcRd( ARRAY_GFED3,fId1,'DMTEMF',st2d,ct2d )
      CALL NcCl( fId1 ) 
     ...
     &         'GFED3_201203/YYYY/3hourly/GFED3_FR_3HR_YYYYMMDDhh'
...
999   CONTINUE
     ...
     &               'GFED3_201203/YYYY/Daily/GFED3_FR_DAY_YYYYMMDD'
...
!
      ...
      ! File name
      FILENAME = '/net/d13/data/kingshuk/GFED4/' //
